...
from ..transforms import (addDPG, affine_transform, flip_joints_3d, batch_rodrigues_numpy, rotmat_to_quat_numpy,
                          get_affine_transform, im_to_torch, flip_thetas, flip_twist, rot_aa)
...
def rotmat_to_aa(rotmat: torch.tensor) -> torch.tensor:
    """
    Borrowed from mmhuman3d
    Convert 3x3 rotation matrix to axis angles
    Args:
        rotmat: rotation matrix of shape
    """
    if rotmat.shape[-1] != 3 or rotmat.shape[-2] != 3:
        raise ValueError(f'Invalid rotation matrix shape f{rotmat.shape}.')
    return matrix_to_axis_angle(rotmat)

class SimpleTransformCamEFT(object):
    ...
    def __init__(self, dataset, scale_factor, color_factor, occlusion, add_dpg,
                 ...
                 train, loss_type='MSELoss', scale_mult=1.25, dict_output=False, bbox_3d_shape=0):
    ...
    def __call__(self, src, label):
        ...
        theta_rot = label['theta'].copy()
        parm_cam = label['parm_cam'].copy()

        # theta (24, 3, 3) -> theta_aa (24, 3)
        theta_rot = torch.from_numpy(theta_rot)
        theta_aa = rotmat_to_aa(theta_rot)
        theta_aa = theta_aa.numpy()

        assert not (theta_aa<1e-3).all(), label
        ...
        joints = gt_joints
        if random.random() > 0.75 and self._train:           # coco2d 0.5
            ...
            # flip theta_aa
            theta_aa = flip_thetas(theta_aa, self._joint_pairs_24)
            twist_phi, twist_weight = flip_twist(twist_phi, twist_weight, self._joint_pairs_24)
            center[0] = imgwidth - center[0] - 1

        # rotate global theta
        theta_aa[0, :3] = rot_aa(theta_aa[0, :3], r)
        theta_rot_mat = batch_rodrigues_numpy(theta_aa)
        ...
        intrinsic_param = np.ones((3, 3)).astype(np.float32)
        ...
        cam_scale = parm_cam[0]
        cam_trans = parm_cam[1:]
