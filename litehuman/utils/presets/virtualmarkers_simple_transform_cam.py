...
class VirtualmarkersSimpleTransformCam(object):
    ...
    def __init__(self, dataset, scale_factor, color_factor, occlusion, add_dpg,
        ...
        self._markers_pairs_67 = dataset.marker_pairs_67
    ...
    def __call__(self, src, label):
        ...
        gt_markers = label['markers_3d']        # (67, 3)
        ...
        self.num_markers = gt_markers.shape[0]

        joints_vis = np.zeros((self.num_joints, 1), dtype=np.float32)
        markers_vis = np.zeros((self.num_markers, 1), dtype=np.float32)
        joints_vis[:, 0] = gt_joints[:, 0, 1]
        markers_vis[:, 0] = gt_markers[:, 0, 1]
        ...
        if random.random() > 0.5 and self._train:
            ...
            markers = flip_joints_3d(markers, imgwidth, self._markers_pairs_67)
        ...
        for i in range(self.num_markers):
            if markers[i, 0, 1] > 0.0:
                markers[i, 0:2, 0] = affine_transform(markers[i, 0:2, 0], trans)

        # generate training targets
        if self._loss_type == 'MSELoss':
            target, target_weight = self._target_generator(joints, self.num_joints)
            target_markers, target_markers_weight = self._target_generator(markers, self.num_markers)
            ...
            target_markers, target_markers_weight = self._integral_target_generator(markers, self.num_markers, inp_h, inp_w)
        ...
        output = {
            ...
            'target_markers': torch.from_numpy(target_markers).float(),
            'target_markers_weight': torch.from_numpy(target_markers_weight).float(),
