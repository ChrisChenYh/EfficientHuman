...
class MixDataset3Cam(data.Dataset):
    CLASSES = ['person']
    ...
    def __init__(self,
        ...
        if train:
            ...
            self.db4 = COCO_EFT_3D(
                cfg=cfg,
                ann_file='coco2014_eft_all_clean.json',
                train=True
            )
        ...
        if train:
            ...
            self.tot_size = (int(2 * max(self._subset_size)))
            print('max_data_size {}'.format(self.tot_size))
            self.partition = [0.3, 0.1, 0.1, 0.2, 0.3]
        else:
