...
class MixDataset2VMCam(data.Dataset):
    CLASSES = ['person']
    ...
    def __init__(self,
        ...
        if train:
            ...
            self.tot_size = (int(cfg.DATASET.SIZE * max(self._subset_size)))
    ...
    def __getitem__(self, idx):
        ...
        if dataset_idx > 0 and dataset_idx < 3:
            ...
            label_vertices_sub4_mask = torch.zeros(54, 3)
            label_xyz_67 = torch.zeros(67, 3)
            label_xyz_67_mask = torch.zeros(67, 3)
            label_uvd_67 = torch.zeros(67, 3)
            label_uvd_67_mask = torch.zeros(67, 3)

            if dataset_idx == 1:
                # COCO
                label_mks_origin = target.pop('target_markers')
                label_mks_mask_origin = target.pop('target_markers_weight')
                assert label_jts_origin.dim() == 1 and label_jts_origin.shape[0] == 17 * 2, label_jts_origin.shape
                assert label_mks_mask_origin.dim() == 1 and label_mks_origin.shape[0] == 67 * 2, label_mks_origin.shape

                label_jts_origin = label_jts_origin.reshape(17, 2)
                label_jts_mask_origin = label_jts_mask_origin.reshape(17, 2)
                
                label_mks_origin = label_mks_origin.reshape(67, 2)
                label_mks_mask_origin = label_mks_mask_origin.reshape(67, 2)

                label_uvd_67[:, :2] = label_mks_origin[:, :2].clone()
                label_uvd_67_mask[:, :2] = label_mks_mask_origin[:, :2].clone()
            ...
            label_vertices_sub4_mask = label_vertices_sub4_mask.reshape(-1)
            label_xyz_67 = label_xyz_67.reshape(-1)
            label_xyz_67_mask = label_xyz_67_mask.reshape(-1)
            label_uvd_67 = label_uvd_67.reshape(-1)
            label_uvd_67_mask = label_uvd_67_mask.reshape(-1)
            ...
            target['target_uvd_67'] = label_uvd_67
            target['target_weight_67'] = label_uvd_67_mask
            target['target_xyz_67'] = label_xyz_67
            ...
            target['loss_uvd_weight_'] = torch.ones(29, 3)
            target['target_weight_vertices_sub4'] = label_vertices_sub4_mask
        else:
