...
from litehuman.utils.pose_utils import (cam2pixel_matrix, pixel2cam_matrix,
...
class HP3D(data.Dataset):
    ...
    def _load_jsons(self):
        ...
        for ann_image, ann_annotations in zip(database['images'], database['annotations']):
            ...
            # file_name: S1/Seq2/images/S1_Seq2_V7/img_S1_Seq2_V7_006791.jpg
            # target_file_name: S1/Seq2/video_7/006791.jpg
            file_name = ann['file_name']
            filename_split_list = file_name.split('/')            
            video_id = filename_split_list[3]
            video_id = video_id.split('_')[2]
            video_id = video_id.split('V')[1]
            video_name = 'video_' + video_id

            # get image name
            img_name = filename_split_list[4]
            img_name = img_name.split('_')[4]

            tar_file_name = os.path.join(filename_split_list[0], filename_split_list[1], video_name, img_name)

            # abs_path = os.path.join(self._root, 'mpi_inf_3dhp_{}_set'.format('train' if self._train else 'test'), ann['file_name'])
            abs_path = os.path.join(self._root, 'mpi_inf_3dhp_{}_set'.format('train' if self._train else 'test'), tar_file_name)
